...
export default function BookingsManagementPage() {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 ${!isDark ? '' : ''}`}>
      ...
      {!isDark && (
        <style>{`
          ...
          :root:not(.dark) body { background: #f7f6fc !important; }
          :root:not(.dark) .min-h-screen { background: var(--theme-bg) !important; border-radius: 0 !important; }
          /* Cards & sections */
          :root:not(.dark) .min-h-screen .rounded-xl:not([class*="bg-indigo"]):not([class*="bg-gradient"]),
          :root:not(.dark) .min-h-screen .rounded-2xl:not([class*="bg-indigo"]):not([class*="bg-gradient"]) { 
            background: #ffffff !important; border-color: var(--theme-line) !important; 
          }
          /* Brand purple for indigo elements */
          :root:not(.dark) .min-h-screen .text-indigo-400,
          :root:not(.dark) .min-h-screen .text-indigo-300 { color: var(--theme-brand) !important; }
          /* Numbers in filter tabs — purple */
          :root:not(.dark) .min-h-screen button span.rounded-full { color: var(--theme-brand) !important; }
          /* Text: white/opacity -> muted/ink */
          :root:not(.dark) .min-h-screen .text-white\\/40,
          :root:not(.dark) .min-h-screen .text-white\\/50,
          :root:not(.dark) .min-h-screen .text-white\\/60 { color: var(--theme-muted) !important; }
          :root:not(.dark) .min-h-screen .text-white\\/70 { color: var(--theme-ink) !important; }
          :root:not(.dark) .min-h-screen .text-white\\/15,
          :root:not(.dark) .min-h-screen .text-white\\/10 { color: var(--theme-muted) !important; }
          /* Buttons: white bg variants -> card bg */
          :root:not(.dark) .min-h-screen [class*="bg-white/"] { background: #ffffff !important; border-color: var(--theme-line) !important; }
          /* Active tab */
          :root:not(.dark) .min-h-screen .bg-indigo-600 { background: var(--theme-brand) !important; color: #fff !important; }
        `}</style>
      )}
      {/* Day/Night Toggle Button */}
      <button
        onClick={toggleTheme}
        className={`fixed top-4 left-4 z-50 w-10 h-10 rounded-xl flex items-center justify-center shadow-lg transition-all ${
          !isDark
            ? "bg-white border border-[#ece9f6] text-[#6C3CE1] hover:bg-[#f3f1fb]"
